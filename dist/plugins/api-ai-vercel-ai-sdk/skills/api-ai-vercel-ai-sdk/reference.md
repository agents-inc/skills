...
const myTool = tool({
  description: 'Tool purpose',                        // Required
  inputSchema: z.object({ /* Zod schema */ }),         // Required
  execute: async (input, options) => result,           // Required
  strict: true,                                        // Optional: strict schema mode
  needsApproval: true,                                 // Optional: human-in-the-loop
  inputExamples: [{ input: { /* example */ } }],       // Optional: guide the model
  toModelOutput: async ({ output }) => ({ /* */ }),     // Optional: custom model-facing output
...
| `convertToCoreMessages()` | `convertToCoreMessages()` | `convertToModelMessages()` |
